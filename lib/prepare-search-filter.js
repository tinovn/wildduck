...
const filterCustomTags = payload.tags; // Lấy tham số mới 'tags'

if (filterCustomTags && typeof filterCustomTags === 'string') {
    // 1. Tách chuỗi thành một mảng các tag
    const tags = filterCustomTags.split(',').map(tag => tag.trim()).filter(tag => tag);

    if (tags.length > 0) {
        // 2. Tạo một mảng các điều kiện Regex, mỗi tag một điều kiện
        const tagConditions = tags.map(tag => {
            const regexPattern = '"tags":\\s*\\[.*"' + escapeRegexStr(tag) + '"';
            return { 'meta.custom': new RegExp(regexPattern, 'i') };
        });

        // 3. Gộp các điều kiện bằng $and
        if (!filter.$and) {
            filter.$and = [];
        }
